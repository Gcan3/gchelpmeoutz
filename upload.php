...
if(isset($_POST["submit"])){
    ...
    if($valid_img == 0){
        ...
        if($req_topic != "" && $req_main != "" && $req_user != ""){
            ...
            // SQL command for inserting the request to the database (preventing any sql injections)
            $stmt = $conn->prepare("INSERT INTO user_request(request_user,request_topic, request_main, request_image, request_country, request_city, request_email) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $req_user, $req_topic, $req_main, $req_img, $country, $city, $email);
            // Execute the sql query and send a success message
            if($stmt->execute()){
